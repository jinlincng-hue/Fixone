...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "fixone-parts-mvp-"));
...
test("公开页面可以打开", async () => {
  ...
  await request(app).get("/map").expect(200).expect(/FixOne配件库地图/);
});
...
test("微信测试登录可无手机号完成店铺草稿、审核和认领流程", async () => {
  ...
  await merchant
    ...
    .type("form")
    .send({ action: "next", province: "湖南省", city: "长沙市", district: "长沙县", town: "FixOne配件库", village: "测试村", address_detail: "老街 9 号", latitude: "28.463", longitude: "113.331" })
    .expect(302);
...
test("乡邻小铺无需执照即可创建，所有发布进入审核队列", async () => {
  ...
  await neighbour
    .post(`/merchant/onboarding/${shopId}/basic`)
    .type("form")
    .send({ action: "next", name: "", town: "FixOne配件库", village: "青河村", description: "自家蔬菜和闲置物品" })
    .expect(302)
