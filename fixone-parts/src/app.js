...
app.use((req, res, next) => {
  ...
  res.locals.currentArea = "FixOne配件库";
  ...
  const markers = [
    {
      ...
      lat: 28.4638705,
      lng: 113.3307214,
      description: "长沙县FixOne配件库行政区公开地图点位，来自 OpenStreetMap Nominatim。",
      href: null
    },
    {
      type: "landmark",
      ...
      lat: 28.461,
      lng: 113.3346,
      description: "FixOne配件库风貌展示点位，具体位置可后续用实测坐标校准。",
      href: null
    },
...
app.post("/publish", requireAuth, async (req, res) => {
  ...
  if (publishIntent) {
    ...
    const village = text(req.user.village, 80) || "FixOne配件库";
...
app.post("/merchant/home-shop/:id/publish", requireMerchant, async (req, res) => {
  ...
  if (publishIntent) {
    ...
    const image = imagePaths[0] || "/assets/images/listing-placeholder.svg";
    const village = text(shop.village, 80) || text(req.user.village, 80) || "FixOne配件库";
...
app.post("/merchant/products", requireMerchant, async (req, res) => {
  ...
  const createdProduct = db.prepare(
    ...
    text(req.body.village, 80) || "FixOne配件库",
...
app.post("/merchant/services", requireMerchant, (req, res) => {
  ...
  const village = text(req.body.village, 80) || req.user.village || "FixOne配件库";
