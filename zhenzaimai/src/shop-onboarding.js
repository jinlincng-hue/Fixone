...
function registerShopOnboarding(app, deps) {
    ...
    if (step === "basic") {
      ...
      if (shop.business_mode === "home_shop") {
        updateShopDraft(shop.id, {
          ...
          village,
          town: text(req.body.town, 80) || "高桥镇",
      ...
      const address = [text(req.body.province, 40), text(req.body.city, 40), text(req.body.district, 40), text(req.body.town, 80) || "高桥镇", village, addressDetail].filter(Boolean).join("");
      const latitude = Number(req.body.latitude);
      const longitude = Number(req.body.longitude);
      updateShopDraft(shop.id, {
        province: text(req.body.province, 40), city: text(req.body.city, 40), district: text(req.body.district, 40), town: text(req.body.town, 80) || "高桥镇", village,
  ...
  app.post("/merchant/onboarding/:id/description-template", requireAuth, (req, res) => {
    ...
    const place = [text(req.body.village, 80) || shop.village, text(req.body.town, 80) || shop.town || "高桥镇"].filter(Boolean).join("·");
    ...
    res.json({ description: `${name}位于${place || "高桥镇"}，主营${category}${serviceText}。欢迎到店或提前联系咨询。` });
  });
  ...
  app.post("/merchant/onboarding/:id/location", requireAuth, (req, res) => {
    ...
    const addressDetail = text(req.body.address_detail, 200);
    const village = text(req.body.village, 80) || shop.village;
    const fullAddress = [text(req.body.province, 40), text(req.body.city, 40), text(req.body.district, 40), text(req.body.town, 80) || "高桥镇", village, addressDetail]
      .filter(Boolean)
      .join("");
  ...
  app.get("/merchant/location-search", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const response = await fetch(`https://nominatim.openstreetmap.org/search?${params.toString()}`, {
        headers: { "user-agent": "gaoqiao-zhenzaimai-mvp/1.0 (local marketplace)" }
      });
